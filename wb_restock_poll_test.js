...
assert.ok(poll.includes('feedbacks?isAnswered=false'), 'poll also lists unanswered reviews');
assert.ok(poll.includes('pickRestockFeedbacks'), 'poll stores restock reviews for reply matching');
...
assert.ok(shared.includes('PATCH') && shared.includes('/api/v1/questions'), 'WB questions are answered with official PATCH');
assert.ok(shared.includes('feedbacks/answer'), 'WB reviews are answered with feedbacks/answer');
assert.ok(apply.includes('looksLikeRestockCard') || shared.includes('looksLikeRestockCard'),
    'replies on Автоотвечик review cards are accepted');
assert.ok(cfg.includes('[functions.telegram-webhook]') && cfg.includes('verify_jwt = false'),
    'live notify webhook is callable without user JWT');
