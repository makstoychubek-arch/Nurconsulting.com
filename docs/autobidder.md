...
**Парсер аукциона не нужен — есть официальная ручка.** `GET /api/advert/v0/bids/recommendations?advertId=&nmId=` (проверено на живой кампании 16.09.2026) отдаёт:
- `base.competitiveBid` / `base.leadersBid` — конкурентная ставка и ставка лидеров по кампании;
- `normQueries[]` — по каждому кластеру три уровня охвата `reachMin` / `reachMedium` / `reachMax` в `bidKopecks` (минорные единицы валюты кабинета).

125 кластеров за 3 секунды, без капчи и лимитов. Подключено: `parseBidRecommendations` в `_shared/wb-cluster-board.ts`, коридор едет вместе с бордом кластеров и виден на экране. Хост `catalog-ads.wildberries.ru` из этой инфраструктуры вообще не резолвится, так что парсер не вариант.
...
**Позицию берём из официального отчёта продавца** `POST /api/v2/search-report/product/orders` (seller-analytics-api), а не сканируя `search.wb.ru`. Реализация: `_shared/autobidder-positions.ts`.

Почему не скан выдачи — проверено 16.09.2026 на артикуле 1218782505:
- пагинация обрывается на 3-й странице: страницы 4+ молча возвращают первую, то есть «просканировано 1000» на деле означает 300 уникальных товаров;
- в анонимной выдаче не видно наших рекламных мест, поэтому позиция систематически занижается;
- быстро прилетает 429, а подпись геоданных (`userDataSign`) глубокую пагинацию не открывает;
- скан показывал «нет в топ-1000» по запросам «свитер» и «свитер женский оверсайз», тогда как официальный отчёт давал 9-е и 22-е место. Скан просто врал.

Отдельная ловушка: выдача зависит от региона (`dest`). Замер по Бишкеку (`dest=286`) не имеет отношения к делу, если товар лежит и продаётся в России — нужен российский `dest`. Валюта кабинета (KGS) региона покупателей не определяет.

Что важно в отчёте:
- берём позицию за **самый свежий день** с данными, а не среднее за период: неделю назад ставка была другой, и среднее уводит биддер не туда;
- `avgPosition = 0` значит «не показывались», а не первое место;
- позицию старше `POSITION_MAX_AGE_DAYS` (3 дня) считаем неизвестной — лучше «не знаю», чем решение по прошлой неделе;
- `frequency = 0` значит, что кластер не ищут: такой ставкой не разогнать, тик его пропускает с причиной `no_demand`;
- лимиты: 30 фраз за запрос, 3 запроса в минуту на кабинет, период ≤ 7 дней. Поэтому позиции кешируются в `adv_position_cache` на `POSITION_CACHE_TTL_MIN` (3 часа) — отчёт всё равно обновляется не чаще раза в час, а тик ходит каждые 5 минут.

**Границы ставки.** `boundsFromCorridor` подставляет коридор WB туда, где правило ничего не задало: нет `max_bid` → потолок берётся из `reachMax`, нет `min_bid_floor` → пол из `reachMin`. Без этого биддер с незаполненным правилом уходил бы вверх без ограничения.

**Рекомендация:** гибрид. Коридор WB даёт стартовую точку и границы, замкнутый контур по позиции корректирует по факту, потому что из-за весовых коэффициентов WB более высокая ставка ≠ гарантированная позиция.
...
FOR each rule IN autobidder_rules WHERE is_active AND schedule_allows_now():
    ...
    my_pos = position_from_report(camp.nm_id, cluster)   -- search-report, кеш 3 ч
...
## 12. Прогон на живых данных 16.09.2026

Кампания 39829721 (артикул 1218782505, кабинет Zevina 1), тик в `dryRun`, цель — позиции 3-10, пол и потолок в правилах не заданы, чтобы проверить подстановку коридора WB.

| кластер | частота | позиция | ставка | пол / потолок из WB | решение |
|---|---|---|---|---|---|
| женские вязаные кофты | 0 | — | 375 | — | `no_demand`, пропущен |
| кофта женская | 169 610 | 19 | 375 | 455 / 1425 | `pos_worse` → 455 |
| свитер | 65 490 | 5 | 375 | 340 / 485 | `in_range`, не трогаем |
| свитер белый женский | 4 069 | 2 | 500 | 330 / 440 | `pos_better` → 440 |

Что это подтверждает:

- позиция реально приходит из отчёта (`bid_history.source = 'report'`), а не из заглушки;
- кластер с нулевой частотой не попадает ни в историю, ни в снимки — просто пропускается;
- «кофта женская» стояла ниже минимума WB (375 против 455) и поднимается ровно до минимума, а не на слепой процент;
- «свитер белый женский» — та самая переплата: позиция 2 при цели 3-10, ставка срезается до `reachMax` = 440.

Отдельно видно, зачем брать последний день, а не среднее: по «кофте женской» среднее за неделю даёт 72-е место, а последний день — 19-е. По среднему биддер задрал бы ставку зря.
