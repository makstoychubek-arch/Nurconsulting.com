...
- **Валюта:** ставки, потолки (`max_bid`, `min_bid_floor`, `outbid_step`) и рекламные бюджеты в Advertising API — в **валюте кабинета продавца**, не обязательно в рублях. У Zevina 1 (ИП Уркунбаев) кабинет в **KGS** (`get-bids` отдаёт `currency: "KGS"`, ставка 420 — это сомы, не рубли). Baza / Elium нужно смотреть по токену того кабинета. Формула `target_drr` смешивает цену товара (часто RUB из юнит-экономики) и ставку — **не считать, пока не сведём валюты**. Список мест, где в тексте ещё «₽»: `docs/autobidder-currency-ruble-sites.md`.
