...
Аудит всех 39 методов спеки vs код: `docs/autobidder-endpoints-audit.md`. Командный центр UI — вкладка **Реклама** рядом с карточками РК; старый Автобиддер не тронут.
...
Официальный кандидат вместо парсера: `GET /api/advert/v0/bids/recommendations` (competitiveBid / коридоры охвата / для CPC — ставки на позиции 1–2, 3–10, 11–34). Пока **не подключён**. Разбор: `docs/autobidder-endpoints-audit.md`.
