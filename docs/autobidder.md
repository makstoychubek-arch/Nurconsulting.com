...
- **Валюта:** ставки, потолки (`max_bid`, `min_bid_floor`, `outbid_step`) и рекламные бюджеты в Advertising API — в **валюте кабинета продавца**, не обязательно в рублях. Baza, Elium и Zevina 1 — все **KGS** (живой `get-bids`, таблица `docs/autobidder-cabinet-currency.md`). Формула `target_drr` смешивает цену товара (часто RUB из юнит-экономики) и ставку — **не считать, пока не сведём валюты**. Список мест, где в тексте ещё «₽»: `docs/autobidder-currency-ruble-sites.md`.
