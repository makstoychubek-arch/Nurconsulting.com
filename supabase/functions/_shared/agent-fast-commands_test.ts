...
Deno.test("wow pulse urgent stock", () => {
  ...
  assertEquals(parseFastCommand("/selfbuy")?.cmd, "selfbuy");
  assertEquals(parseFastCommand("самовыкуп")?.cmd, "selfbuy");
  assertEquals(parseFastCommand("самовыкуп клиент ключ"), null);
});
