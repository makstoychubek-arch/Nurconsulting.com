...
export function isLikelyFollowUp(text: string): boolean {
  ...
  if (
    /(блузк|лапш|водолазк|гольф|фонар|вырез|укороч|костюм|пиджак|жакет|жилет|^жл|бомбер|кимоно|плать|рубашк|кардиган|свитер|худи|юбк|двойк|спорт|велюр|комбинезон|черн|чёрн|бел|беж|бордо|графит|коричнев|темн|изумруд|хаки|пудр|молочн)/i
      .test(t)
  ) {
    return true;
  }
