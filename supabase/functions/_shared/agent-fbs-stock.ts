...
export function wantsFbsSizeTable(text: string): boolean {
  return /(сводн|таблиц|по\s*размер|размер(ам|ов|ы)|красив[а-яё]*\s*(фото|таблиц)|фото\s*(свод|таблиц|остат))/i
    .test(text || '');
}
...
function extractProductText(text: string): string {
  ...
  const cleaned = text
    .replace(/@\w+/g, ' ')
    .replace(/(^|[\s,.:;!?])(антон|anton|логист[а-яё]*)(?=$|[\s,.:;!?])/gi, ' ')
    .replace(/(^|[\s,.:;!?])(фбс|fbs)(?=$|[\s,.:;!?])/gi, ' ')
    .replace(
      /(^|[\s,.:;!?])(остат[а-яё]*|осталось|сколько|налич[а-яё]*|склад[а-яё]*|кабинет[а-яё]*|слыш[а-яё]*|дай|скинь|покажи|нужен|нужно|есть|все|всех|сейчас|сейча|вб|wb|wildberries|вайлд[а-яё]*|маркетплейс[а-яё]*|продавц[а-яё]*)(?=$|[\s,.:;!?])/gi,
      ' ',
    )
