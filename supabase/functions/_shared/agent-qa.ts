...
export function wantsArticleLookup(text: string): boolean {
  ...
  if (wantsPriceChange(t)) return false;
  if (
    /(дай|скинь|пришли|покажи|напиши|скажи|какой|какая|какие|нужен|нужна|кинь).{0,36}(артикул|арт\.?|\bnm\b|нм\b)/i
      .test(t)
  ) {
    return true;
  }
  if (
    /(артикул|арт\.?|\bnm\b|нм\b).{0,36}(дай|скинь|пришли|покажи|напиши|скажи|какой|какая|на\s|нужен)/i
      .test(t)
  ) {
    return true;
  }
  // без JS \b — после кириллицы граница не срабатывает
  if (/^(какой\s+)?(артикул|арт|nm|нм)(?=$|[\s,.:;!?/\\|«»"'])/i.test(t.trim())) {
    return true;
  }
  return false;
}
