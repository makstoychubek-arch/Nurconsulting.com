...
export function scoreProductMatch(haystack: string, query: string): number {
  ...
  const types: Array<[RegExp, RegExp, number]> = [
    ...
    [/блузк?/, /блуз/, 3],
    [/укороч|crop|кроп/, /укороч|crop|кроп/, 6],
    ...
    [/пальто|тренч|плащ/, /пальто|тренч|плащ/, 5],
    [/оверсайз|oversiz/, /оверсайз|oversiz|over/, 3],
    [/поло(?![а-я])/, /поло/, 5],
    [/джинс/, /джинс/, 4],
  ];
  ...
  const colors: Array<[RegExp, RegExp, number]> = [
    ...
    [/графит|сер(ый|ая|ое|ые)?/, /графит|сер|граф/, 3],
    ...
    [/голуб/, /голуб/, 4],
    [/розов|пудр/, /розов|пудр/, 4],
    [/зелен|изумруд|ментол|хаки|оливк/, /зелен|изумруд|ментол|хаки|оливк/, 4],
    [/желт|горчиц/, /желт|горчиц/, 4],
    [/фиолет|лилов|сирен/, /фиолет|лилов|сирен/, 4],
    [/айвори|кремов|молочн/, /айвори|кремов|молочн/, 4],
    [/красн|алый|scarlet|терракот|оранж/, /красн|алый|терракот|оранж/, 4],
