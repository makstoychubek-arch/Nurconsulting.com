...
export function extractNmId(text: string): number | undefined {
  ...
  const labeled = t.match(
    /\b(?:арт(?:икул)?\.?|nm\.?|nmid|тест(?:а|у|ом)?|карточк\w*|товар\w*|sku)\s*[:#№]?\s*(\d{5,12})\b/i,
  );
  if (labeled) return Number(labeled[1]);
  const afterVerb = t.match(
    /\b(?:отч[её]т|ротац\w*|смен\w*|переключ\w*|результат\w*|статус|как\s+там)\s+(?:по\s+)?(?:арт\.?\s*)?(\d{5,12})\b/i,
  );
  if (afterVerb) return Number(afterVerb[1]);
  // одиночный артикул в коротком сообщении
  const bare = t.match(/^\s*(?:арт\.?\s*)?(\d{5,12})\s*[?.!]?\s*$/i);
  if (bare) return Number(bare[1]);
  const any = t.match(/\b(\d{6,12})\b/);
  if (any && !/\b\d{1,2}[./]\d{1,2}/.test(t)) return Number(any[1]);
  return undefined;
}
...
export function parseAbIntent(raw: string): AbParsed {
  ...
  // как запустить
  if (
    /(как\s+(запуст|запуск|создать|сделать|начать)\w*|запусти\s+тест|хочу\s+запустить|новый\s+тест|создай\s+тест)/.test(
      lower,
    )
  ) {
  ...
  // ротация
  if (
    /(ротац|смени\s+фото|поменяй\s+фото|переключ\w*\s+(вариант|фото)|следующ\w*\s+(вариант|фото)|крутани|прокрути|ротани)/.test(
      lower,
    )
  ) {
  ...
  // список
  if (
    /(тесты|какие\s+тесты|что\s+крутится|активн\w*\s+тест|список\s+тест|что\s+тестиру|покажи\s+тест|все\s+тест|а\s*\/\s*б|ab\s*тест)/.test(
      lower,
    ) ||
...
const NEED_NM_VARIANTS = (what: string) =>
  [
    `Нужен артикул. Например: <code>${what} 123456789</code>`,
    `Уточни nm/арт — без номера не пойму, какой тест. Пример: <code>${what} 123456789</code>`,
    `Какой артикул? Напиши число или «${what} 123…»`,
    `Добавь артикул к запросу: <code>${what} &lt;арт&gt;</code>`,
    `Без артикула не попадаю. Формат: <code>${what} 123456789</code>`,
  ] as const;
...
const UNKNOWN_VARIANTS = [
  'Не поняла запрос. Напиши «помощь» или «тесты».',
  'Уточни: список, отчёт, статус или ротация? Можно «помощь».',
  'Это мимо А/Б-команд. Пример: <code>тесты</code> или <code>отчёт 123</code>.',
  'Давай проще: «какие тесты» / «отчёт &lt;арт&gt;» / «смени фото &lt;арт&gt;».',
  'Не распознала. «помощь» — подскажу формулировки.',
] as const;
