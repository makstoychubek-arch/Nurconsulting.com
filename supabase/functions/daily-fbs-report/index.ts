...
import {
    aggregateByCabinet,
    aggregateByModel,
    renderFbsSummaryImage,
} from '../_shared/fbs-summary-image.ts';
import {
    karinaFbsCaption,
...
import {
    fbsCabinetLabel,
    sortFbsCabinets,
} from '../_shared/fbs-cabinet-labels.ts';
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const activeNames = sortFbsCabinets(
            (activeRows || []).map((r) => String(r.cabinet)),
        );
        ...
        // Агрегация для Excel (с кабинетом — иначе Baza/Elium/SAAI смешиваются)
        ...
        for (const r of allRows) {
            const key = `${r.cabinet}|${r.barcode}|${r.productName}|${r.size}`;
        ...
        const sections = aggregateByCabinet(excelRows, activeNames);
        // пустые активные кабинеты тоже показываем в сводке
        for (const name of activeNames) {
            if (!sections.some((s) => s.cabinet === name)) {
                sections.push({ cabinet: name, models: [], qty: 0 });
            }
        }
        sections.sort(
            (a, b) =>
                activeNames.indexOf(a.cabinet) - activeNames.indexOf(b.cabinet),
        );
        const models = aggregateByModel(excelRows);
        const totalQty = excelRows.reduce((a, r) => a + r.qty, 0);
        const cabinetLines = activeNames.map((name) => {
            const st = cabinetStats.find((s) => s.cabinet === name);
            const orders = Number(st?.orders ?? 0);
            const err = st?.error ? String(st.error) : '';
            const label = fbsCabinetLabel(name);
            if (err) return `${label}: ошибка`;
            return `${label}: ${orders} шт`;
        });

        const png = await renderFbsSummaryImage(pretty, sections);
        const caption = karinaFbsCaption({
            prettyDate: pretty,
            totalQty,
            modelsCount: models.length,
            failedCabinets: failedCabinets.map(fbsCabinetLabel),
            cabinetLines,
        });
