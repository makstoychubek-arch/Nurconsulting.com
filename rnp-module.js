...
const RNP = (() => {
    ...
    // Диапазоны vol → номер basket-хоста WB. Таблица верна до vol 4997; дальше
    // WB давно нарезает хосты по-своему, а прежняя экстраполяция «по 216 vol»
    // уходила всё дальше от правды: для vol 15444 она давала basket-76, которого
    // вообще нет в DNS, — в РНП вместо фото оставалась заглушка.
    // Опорные точки ниже сняты запросами к CDN 16.09.2026 (см. вол/хост в
    // кабинетах Baza/Zevina/Elium). Настоящий источник — photo_url из карточки
    // WB, эта функция нужна лишь пока фото у артикула не заполнено.
    // [первый известный vol хоста, номер хоста] по возрастанию.
    const _BASKET_ANCHORS = [
    ...
    // Шаг последних измеренных хостов: (15444 − 12402) / (48 − 44).
    const _BASKET_TAIL_STEP = 760;
    ...
    function _basketNum(vol) {
        ...
        const last = _BASKET_ANCHORS[_BASKET_ANCHORS.length - 1];
        if (vol >= last[0]) return last[1] + Math.round((vol - last[0]) / _BASKET_TAIL_STEP);
        let host = _BASKET_ANCHORS[0][1];
        for (const [anchorVol, anchorHost] of _BASKET_ANCHORS) {
            if (vol < anchorVol) break;
            host = anchorHost;
        }
        return host;
    }
